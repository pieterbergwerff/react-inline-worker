...
function HelloWorldExample() {
  return react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(Example, {
    ...
    code: ["import React from 'react';", "import InlineWorker from 'react-inline-worker';", "", "function fn(){", "  postMessage('Hello world!');", "}", "", "function HelloWorldExample(){", "  return <InlineWorker fn={fn}>{({ loading, error, data }) => {", "    if (loading) return 'loading...';", "    if (error) return 'Errors!';", "    console.log(data); // Hello world!", "    return <div>Message from worker: {data}</div>;", "  }}</InlineWorker>;", "}", "", "export default HelloWorldExample;"].join('\n'),
...
function FetchApiExample() {
  return react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(Example, {
    ...
    description: "This example fetches your IP address from ipify.org.",
    code: "var test2 = false;",
    RunComponent: function RunComponent(_ref4) {
      var onReady = _ref4.onReady;

      function fn() {
        fetch('https://api.ipify.org?format=json').then(function (data) {
          console.log(data);
          postMessage('Hello world!');
        }).catch(function (err) {
          console.error('Unable to fetch ipify.org API!');
          postMessage(err);
        });
      }
...
//# sourceMappingURL=index.js.f4643d2ec35da4aae0a5.hot-update.js.map